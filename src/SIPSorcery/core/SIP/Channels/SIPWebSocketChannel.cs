    ...
    public class SIPWebSocketChannel : SIPChannel
    {
            ...
            bool? ignoreExtensionsOverride) : base(sipEncoding, sipBodyEncoding)
        {
            ...
            logger.LogInformation(
                "SIP WebSocket server starting for {EndPoint}. Scheme={Scheme} IgnoreExtensions={IgnoreExtensions}.",
                endPoint,
                IsSecure ? "wss" : "ws",
                _ignoreExtensionsOverride.HasValue ? _ignoreExtensionsOverride.Value.ToString() : "default");

            Action<SIPMessagWebSocketBehavior> configureBehavior = (behaviour) =>
            {
            ...
            m_webSocketServer.AddWebSocketService<SIPMessagWebSocketBehavior>("/", configureBehavior);

            m_webSocketServer.Start();
            logger.LogInformation(
                "SIP WebSocket server started for {EndPoint}. Secure={Secure} ActiveConnections={ActiveConnections}.",
                endPoint,
                IsSecure,
                m_ingressConnections.Count);
        }
