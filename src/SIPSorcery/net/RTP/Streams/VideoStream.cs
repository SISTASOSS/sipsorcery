...
namespace SIPSorcery.Net
{
    public class VideoStream : MediaStream
    {
        ...
        private Vp8PacketReorderBuffer _vp8PacketReorderBuffer;
        ...
        public void ProcessVideoRtpFrame(IPEndPoint endpoint, RTPPacket packet, SDPAudioVideoMediaFormat format)
        {
            ...
            if (RtpVideoFramer == null)
            {
                if (videoFormat.Codec == VideoCodecsEnum.VP8 ||
                    videoFormat.Codec == VideoCodecsEnum.VP9 ||
                    videoFormat.Codec == VideoCodecsEnum.AV1 ||
                    videoFormat.Codec == VideoCodecsEnum.H264 ||
                    videoFormat.Codec == VideoCodecsEnum.H265 ||
                    videoFormat.Codec == VideoCodecsEnum.JPEG)
                {
                    logger.LogDebug("Video depacketisation codec set to {Codec} for SSRC {SSRC}.", videoFormat.Codec, packet.Header.SyncSource);

                    RtpVideoFramer = new RtpVideoFramer(videoFormat.Codec, MaxReconstructedVideoFrameSize);
                    _vp8PacketReorderBuffer = videoFormat.Codec == VideoCodecsEnum.VP8
                        ? new Vp8PacketReorderBuffer()
                        : null;
                }
                else
                {
            ...
            ProcessPacketThroughFramer(endpoint, packet, videoFormat);
        }

        private void ProcessPacketThroughFramer(IPEndPoint endpoint, RTPPacket packet, VideoFormat videoFormat)
        {
            if (videoFormat.Codec == VideoCodecsEnum.VP8)
            {
                _vp8PacketReorderBuffer ??= new Vp8PacketReorderBuffer();
                var reorderResult = _vp8PacketReorderBuffer.ProcessPacket(packet, DateTime.UtcNow);

                if (reorderResult.FrameResetRequired)
                {
                    RtpVideoFramer?.Reset();
                    logger.LogDebug(
                        "Resetting VP8 frame assembly after reorder timeout/overflow. SSRC {SSRC}, SeqNum {SequenceNumber}, Buffered {BufferedPackets}.",
                        packet.Header.SyncSource,
                        packet.Header.SequenceNumber,
                        reorderResult.BufferedPacketCount);
                }

                foreach (var orderedPacket in reorderResult.ReleasedPackets)
                {
                    EmitFramedVideo(endpoint, orderedPacket, videoFormat);
                }

                return;
            }

            EmitFramedVideo(endpoint, packet, videoFormat);
        }

        private void EmitFramedVideo(IPEndPoint endpoint, RTPPacket packet, VideoFormat videoFormat)
        {
            var frame = RtpVideoFramer.GotRtpPacket(packet);
            if (frame != null)
            {
                OnVideoFrameReceivedByIndex?.Invoke(Index, endpoint, packet.Header.Timestamp, frame, videoFormat);
            }
        }
